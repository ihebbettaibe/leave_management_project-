...
-- Customer Support Team Profiles
...
SELECT 
    u.id as user_id,
    2 as leave_type_id, -- Sick Leave
    2024 as year,
    0 as carryover,
    FLOOR(RANDOM() * 8 + 1) as used -- 1-8 sick days used
FROM users u;

-- 2025 balances with some carryover from 2024
INSERT INTO leave_balances (user_id, leave_type_id, year, carryover, used) 
SELECT 
    u.id as user_id,
    1 as leave_type_id, -- Annual Leave
    2025 as year,
    GREATEST(0, 25 - lb.used - 5) as carryover, -- Carryover unused days (max 20)
    FLOOR(RANDOM() * 5 + 1) as used -- 1-5 days used so far in 2025
FROM users u
...
SELECT 
    ...
    2 as leave_type_id, -- Sick Leave
    2025 as year,
    0 as carryover, -- Sick leave doesn't carry over
    FLOOR(RANDOM() * 3 + 0) as used -- 0-2 sick days used so far in 2025
FROM users u;
