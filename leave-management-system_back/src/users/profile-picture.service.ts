...
export class ProfilePictureService {
  ...
  async uploadProfilePicture(userId: string, file: Express.Multer.File): Promise<string> {
    try {
      ...
      const result = await this.dataSource.query(query, [profilePicUrl, userId]);
      ...
      console.log(`✅ Profile picture uploaded for user ${userId}: ${profilePicUrl}`);
      ...
      if (verifyResult && verifyResult.length > 0) {
        console.log('✅ Database updated successfully. New profile_picture_url:', verifyResult[0].profile_picture_url);
      } else {
